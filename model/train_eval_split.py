...
def validation_unet_split(model, loader, nonwater=0, water=1, device='cuda:0', loss_f='BCE', water_threshold=0.5):
    '''
    ...
    with torch.no_grad(): # specifies gradient is not computed
        ...
        for batch in loader:
            ...
            binary_predictions = (predictions >= water_threshold).float()
            #combine reaches prediction and target
            print(f"prediction shape = {predictions.shape}")
            print(f"target shape = {target.shape}")

            top_binary_predictions = binary_predictions[::2,:,:]
            bottom_binary_predictions = binary_predictions[1::2,:,:]
            combined_binary_predictions = torch.concatenate([top_binary_predictions,bottom_binary_predictions],dim=1)
            
            top_predictions = predictions[::2,:,:]
            print(f"prediction top shape = {top_predictions.shape}")
            bottom_predictions = predictions[1::2,:,:]
            print(f"prediction bot shape = {bottom_predictions.shape}")
            combined_predictions = torch.concatenate([top_predictions,bottom_predictions],dim=1)
            print(f"prediction combined shape = {combined_predictions.shape}")

            top_target = target[::2,:,:]
            bottom_target = target[1::2,:,:]
            combined_target = torch.concatenate([top_target,bottom_target],dim=1)
            
            # compute loss 
            loss = choose_loss(combined_predictions, combined_target, loss_f)
            # compute metrics
            accuracy, precision, recall, f1_score, csi_score = compute_metrics(combined_binary_predictions, combined_target, nonwater, water)
