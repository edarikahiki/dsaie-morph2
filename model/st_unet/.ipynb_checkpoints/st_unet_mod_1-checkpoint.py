...
class Down3D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, temporal_kernel=1, drop_channels=False, p_drop=None, 
        ...
        self.conv = DoubleConv3D(in_channels, out_channels, kernel_size=kernel_size, temporal_kernel=temporal_kernel,
...
class Up3D(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        # collapse skip temporal dimension if needed
        if x2.size(2) > 1:
...
class UNet25D_bottleneck(nn.Module):
    def __init__(self, n_channels, n_classes, init_hid_dim=16, kernel_size=3, temporal_kernel=1, pooling='max', bilinear=False,
        ...
        self.inc = DoubleConv3D(n_channels, hid_dims[0], kernel_size=kernel_size, drop_channels=drop_channels, p_drop=p_drop)

        self.down1 = Down3D(hid_dims[0], hid_dims[1], kernel_size=kernel_size, temporal_kernel=1, 
                            drop_channels=drop_channels, p_drop=p_drop, pool_temporal=pool_temporal, pooling=pooling)
        self.down2 = Down3D(hid_dims[1], hid_dims[2], kernel_size=kernel_size, temporal_kernel=1, 
                            drop_channels=drop_channels, p_drop=p_drop, pool_temporal=pool_temporal, pooling=pooling)
        self.down3 = Down3D(hid_dims[2], hid_dims[3], kernel_size=kernel_size, temporal_kernel=1, 
                            drop_channels=drop_channels, p_drop=p_drop, pool_temporal=pool_temporal, pooling=pooling)
        self.down4 = Down3D(hid_dims[3], hid_dims[4], kernel_size=kernel_size, temporal_kernel=temporal_kernel, 
                            drop_channels=drop_channels, p_drop=p_drop, pool_temporal=pool_temporal, pooling=pooling)
