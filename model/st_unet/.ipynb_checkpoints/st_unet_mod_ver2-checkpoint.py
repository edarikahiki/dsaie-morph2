...
class UNet25D(nn.Module):
    def __init__(self, n_channels, n_classes, init_hid_dim=16, kernel_size=3, temporal_kernel=1, pooling='max', bilinear=False,
        ...
        self.up1 = Up2D(hid_dims[4], hid_dims[3], kernel_size, bilinear, drop_channels, p_drop)
        self.up2 = Up2D(hid_dims[3], hid_dims[2], kernel_size, bilinear, drop_channels, p_drop)
        self.up3 = Up2D(hid_dims[2], hid_dims[1], kernel_size, bilinear, drop_channels, p_drop)
        self.up4 = Up2D(hid_dims[1], hid_dims[0], kernel_size, bilinear, drop_channels, p_drop)
