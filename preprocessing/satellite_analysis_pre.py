...
def rename_images(collection, train_val_test, reach=1, dir=r'data/satellite/original_30'):
    ''' 
    ...
    directory = os.path.join(dir, collection.replace('/', '_'))
    if train_val_test is not None and reach is not None:
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(f"Directory not found: {os.path.abspath(path)}\nMake sure you're in the correct working directory and the folder exists.")
...
def list_paths(collection, train_val_test, reach=1, dir_orig=r'data/satellite/original_30'):
    '''
...
def reshape_images(image, desired_shape=(2000,1000), new_padded_class=None):
    '''
...
def preprocess_images(input_path, desired_shape=(2000,1000), reshape_img=True, 
...
def save_tot_pixels(train_val_test, reach=1, nodata_value=-1, nonwater_value=0, water_value=1,
                    directory=r'data/satellite/preprocessed_30', collection=r'JRC_GSW1_4_MonthlyHistory'):
    '''
...
def season_average(train_val_test, reach, year, dir_datasets=r'data/satellite', nodata=-1):
    ...
    for month in range(1, 5):
        ...
        folder = os.path.join(dir_datasets, fr'dataset_month{month}_30')
        # get all reaches
            ...
            if reach_folder.endswith(f'{train_val_test}_r{reach}'):
                ...
                for image in os.listdir(reach_path):
                    if image.startswith(f'{year}'):
                        ...
                        img = np.where(img[0][:] == nodata, np.nan, img[0][:]) # replace nodata with nan
                        imgs.append(img)
...
def export_good_avg(train_val_test, reach, years=[1988, 2021], dir_datasets=r'data/satellite', 
                    dir_output=r'data/satellite/averages_30', nodata=-1, window_size=15, replace_default=0):
    '''
...
def load_avg(train_val_test, reach, year, dir_averages=r'data/satellite/averages_30'):
    ''''
...
def load_df(collection, cloud_cov=100, dir_files=r'data/satellite/original_30', print_total=True, old=False):
    '''
