...
dir_orig = r'data/satellite/original_30' # original images
dir_proc = r'data/satellite/preprocessed_30' # preprocesed images
...
def show_image_array(path, scaled_classes=True, cmap='gray', vmin=0, vmax=2, show=False, save_img=False):
    '''
    ...
    # print("Array dtype before cast:", img_array.dtype)
    # print(img_array.flat[:10])  # first 10 values
...
def load_all_csv(train_val_test, cols=['Date image', 'no-data: 0', 'non-water: 1', 'water: 2'], dir_folders=r'data/satellite/preprocessed_30', index_col=None):
    '''
...
def create_long_df(train_val_test, cols=['Date image', 'no-data: 0', 'non-water: 1', 'water: 2'], dir_folders=r'data/satellite/preprocessed_30'):
    '''
...
def get_info_images(df, acc_percentage, shape=(2000,1000)):
    '''
...
def clear_full_df(train_val_test, nodata_thr, water_thr, cols=['Date image', 'no-data: 0', 'non-water: 1', 'water: 2'], 
             dir_folders = r'data/satellite/preprocessed_30', dir_output = r'cleared_csv'):
    '''
...
def clear_single_df(train_val_test, nodata_thr, water_thr, dir_folders = r'data/satellite/preprocessed_30'):
    '''
...
def compute_diff(train_val_test, reach_id, dir_folders = r'data/satellite/preprocessed_30', collection = r'JRC_GSW1_4_MonthlyHistory'):
    '''
...
def assign_group(df, monsoon_start=5, monsoon_end=10):
    ...
    for i in range(len(df)):
        ...
        if df['Date image'].iloc[i].month >= monsoon_start and df['Date image'].iloc[i].month <= monsoon_end:
            df.loc[df.index[i], 'Group'] = f"Monsoon {year - df.loc[df.index[0], 'Date image'].year}"
        ...
        # apply group i to images between monsoon_end month of year i 
            ...
            if df['Date image'].iloc[i].month == start_low_flow: # sets group value based on previous_group update
                df.loc[df.index[i], 'Group'] = previous_group // 2
                 

            else: # makes sure that group value is costant across all months of the same period 
                df.loc[df.index[i], 'Group'] = df.loc[df.index[i-1], 'Group']
...
def load_df_countpixels(train_val_test, reach_id, monsoon_start=None, monsoon_end=None, single_month=None, dir_folders = r'data/satellite/preprocessed_30', 
...
def copy_images(train_val_test, reach_id,  monsoon_start=None, monsoon_end=None, single_month=None, dir_folders = r'data/satellite/preprocessed_30', 
                dir_dest = r'data/satellite/dataset_30', collection = r'JRC_GSW1_4_MonthlyHistory', 
